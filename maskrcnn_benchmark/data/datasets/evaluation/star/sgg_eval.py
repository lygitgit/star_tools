...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        # TODO: pred_triplets是由物体类别组成的三元组，pred_rels是目标的id
...
def _compute_pred_matches(gt_triplets, pred_triplets,
    ...
    # TODO:keeps是筛选出来三元组中和gt，主语类型一致、目标间关系一致、客体类型一致的对应组合
    keeps = intersect_2d_optimized(gt_triplets, pred_triplets)
    # keeps = intersect_2d(gt_triplets, pred_triplets)
    gt_has_match = keeps.any(1)
    # TODO:找出有对应的gt所处的keeps，对其中类型能对应上的求IOU交并比，判断是否为真
...
def intersect_2d_optimized(x1, x2):
    """
    使用 `np.in1d` 加速两个大数组的交集计算。
    ...
    # result = np.equal(x1[:, None], x2)  # x1[:, None] 将 x1 从 (m1, n) 扩展为 (m1, 1, n)

    x1_tensor = torch.tensor(x1[:, None])  # 或者直接从其他来源转换为张量
    x2_tensor = torch.tensor(x2)

    # 使用 torch.eq 来替代 np.equal
    result = np.array(torch.eq(x1_tensor, x2_tensor).all(axis=-1))
    return result  # 在列轴上比较是否完全相等

def intersect_2d_optimized_sum(x1, x2):
    """
    使用 `np.in1d` 加速两个大数组的交集计算。
    :param x1: [m1, n] numpy array
    :param x2: [m2, n] numpy array
    ...
    if x1.shape[1] != x2.shape[1]:
        raise ValueError("Input arrays must have the same number of columns")
    # STAR中目标类型和关系类型都不超过60
    x1_sqeeze = x1[:, 0] * 3600 + x1[:, 1] * 60 + x1[:, 2]
    x2_sqeeze = x2[:, 0] * 3600 + x2[:, 1] * 60 + x2[:, 2]
    result = (x1_sqeeze[:, None] == x2_sqeeze[None, :])

    return result.all(axis=-1)  # 在列轴上比较是否完全相等
